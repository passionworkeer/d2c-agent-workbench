...
- 以上为默认门槛；真实手机截图样例（照片重采样 + 语义重建导航的像素对比天花板实测 75-80 分）由服务端 Profile 按样例声明验收门槛 `70/62`（`apps/server/src/profiles.ts` 的 `acceptance`），分数本身不变，P0/P1 硬门槛也不随验收门槛放松。
...
**当前落地（2026-08-30）**：已有 5 个黄金样例——`campaign`（主视觉演示骨架，内置可修复基线间距问题）、`summer-form`（表单演示骨架），以及 3 个真实手机截图样例 `commerce-feed` / `summer-game-festival` / `pet-red-packet`（方案 C 混合重建：原图作整页图集素材 + 语义化组件重建导航/任务/奖励区，本地演示交互不接真实后台；保真范围明示为 390px 手机端整页，桌面端不承诺）。真实样例的整页几何由 E2E 从渲染 Artifact 回读逐节点对照 spec sourceBox（3% 容差）复核。语义评审接 MiniMax 双图实时评审，未配 key 时如实回退服务端注册基准分（provider 区分）。剩余 7 页沿同一 fixture 结构扩充。
