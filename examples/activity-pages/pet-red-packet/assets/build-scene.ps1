...
function Patch-Texture($x,$y,$w,$h,$sx,$sy,$sw=$w,$sh=$h,$feather=3) {
  ...
  for($py=0;$py -lt $patch.Height;$py++){ for($px=0;$px -lt $patch.Width;$px++){
    ...
    if($edge -lt $feather*$scale){ $color=$patch.GetPixel($px,$py); $alpha=[int](255*$edge/($feather*$scale)); $patch.SetPixel($px,$py,[System.Drawing.Color]::FromArgb($alpha,$color)) }
  }}
...
Patch-Texture 96 601 198 29 96 599 198 1 0
