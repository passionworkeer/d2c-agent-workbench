...
/** 养萌宠红包养成页。atlasUrl 指向本样例的参考图整图。 */
export function PetRedPacketExperience({ atlasUrl, ...root }: { atlasUrl: string } & ComponentPropsWithoutRef<"div">) {
    ...
    <div data-activity-canvas-root="" {...root}>
      <MobileActivityShell height={819} pageBackground="#17181f" canvasBackgroundColor="#5c1503">
        <KwaiTopNavigation
          ...
          tabs={[
            { id: "pet-tab-pet", label: "养萌宠" },
            { id: "pet-tab-follow", label: "关注" },
            { id: "pet-tab-mall", label: "商城" },
            { id: "pet-tab-discover", label: "发现" },
            { id: "pet-tab-local", label: "同城" },
          ]}
        />
        ...
        <section style={box(0, 42, 390, 102)} className={pageStyles.petHero}>
          ...
          <div style={box(54, 77, 283, 22)} className={pageStyles.petNotice}>
            <span data-d2c-node-id="pet-hero-notice-text" style={box(6, 4, 271, 14)}>活动时间结束后，宠物玩法即将焕新回归</span>
          </div>
        ...
        <section data-d2c-node-id="pet-stage" style={box(0, 234, 390, 369)}>
          ...
          <span data-d2c-node-id="pet-hint-bubble" style={box(161, 315, 101, 13)} className={pageStyles.petHintBubble}>喂食必得装扮</span>

          <FloatingAction nodeId="pet-signin" badge="+10" label="每日签到" icon="📅" style={box(265, 14, 50, 50)} onClick={() => setToast("签到成功，金币 +10")} />
          <FloatingAction nodeId="pet-summer-night" label="超级夏晚" icon="🎪" style={box(51, 18, 46, 48)} onClick={() => setToast("超级夏晚（本地演示）")} />
          <FloatingAction nodeId="pet-level-claim" label="18级领取" icon="🔮" style={box(257, 68, 53, 50)} onClick={() => setToast("达到 18 级后可领取")} />
          <FloatingAction nodeId="pet-wardrobe" label="我的衣橱" icon="👗" style={box(9, 83, 42, 48)} onClick={() => setToast("我的衣橱（本地演示）")} />
          <FloatingAction nodeId="pet-claim" label="点击领取" icon="🎁" style={box(19, 130, 40, 50)} onClick={() => setToast("礼物已领取（本地演示）")} />
          ...
          <button type="button" data-d2c-node-id="pet-blindbox" className={pageStyles.petBlindbox} style={box(308, 162, 64, 64)} onClick={() => setToast("开盲盒赢现金（本地演示）")}>
            <span data-d2c-node-id="pet-blindbox-title" style={box(8, 8, 48, 14)}>惊喜？</span>
          ...
          {PET_TASKS.map((task) => (
            <TaskCard
              ...
              onAction={() => {
                ...
                setToast(`${task.title}完成，能量 +${task.energyGain}`);
              }}
            />
          ))}
        </section>

        <BottomTabBar publishBadge="23" height={53} />
